...
// Fix1: recalculate the style while changing the pizza size
//remove the determineDx function, and directly use the sizeSwitcher function to return the relative width
...
//Fix2: recalculate the style and layout while scrolling 

//Define a new function preUpdatePositions, to read the needed style property first before updating the position. 
var basicleft = [];
var items;
function preUpdatePositions() {
	items = document.getElementsByClassName('mover');
	//the basicLeft property is fix for each column pizza
	for (var j=0; j<8; j++) {
		basicleft.push(items[j].basicLeft);
	}
}
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    items[i].style.left = basicleft[i%8] + 100 * phase + 'px';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //based on the browser window height to generate the moving pizza number dynamically
  var NumberofMovingPizza = (Math.floor(window.innerHeight/s) + 1)*cols;
  for (var i = 0; i < NumberofMovingPizza; i++) {
  ...
  //add a new function preUpdatePositions to fetch the  information out of the loop
  preUpdatePositions()
  updatePositions();
});
